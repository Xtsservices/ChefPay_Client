...
const EditMenuModal: React.FC<EditMenuModalProps> = ({
  ...
  const handleSubmit1 = async (): Promise<void> => {
    ...
    try {
      const payload = {
        ...
        items: formData.selectedItems.map((id) => ({
          itemId: id,
          days:
            formData.menuType === "day-specific"
              ? formData.selectedDays
              : daysOfWeek.map((d) => d.key),
        })),
      };
  ...
  // Fetch menu by canteenId
  const fetchMenuByCanteenId = async () => {
    try {
      const response = await fetch(
        `http://172.16.4.186:3100/api/menus/getMenuByCanteenID/${
          currentUserData?.canteenId || 1
        }`
      );
      ...
      if (result?.data) {
        ...
        // Extract unique days across all items
        const allDays = [
          ...new Set(menu.items.flatMap((item: any) => item.days)),
        ];

        // Extract itemIds
        const allItemIds = menu.items.map((item: any) => item.itemId);
        setMenuId(menu.id);
        setFormData({
          menuName: "", // API doesn’t provide name, keep optional empty
          menuType: menu.menuType,
          selectedDays: menu.menuType === "day-specific" ? allDays : [],
